...
Supported stdint.h. / Jul 01, 2020 R0.01d Fixed wrong integer type usage. / May
08, 2021 R0.02  Supported grayscale image. Separated configuration options. /
Jun 11, 2021 R0.02a Some performance improvement. / Jul 01, 2021 R0.03  Added
...
JRESULT jd_restart(JDEC *jd,     /* Pointer to the decompressor object */
...
JRESULT jd_mcu_load(JDEC *jd /* Pointer to the decompressor object */
) {
  int32_t *tmp =
      (int32_t *)
            ...
            (JD_USE_SCALE &&
             jd->scale ==
                 3)) { /* If no AC element or scale ratio is 1/8, IDCT can be
                          omitted and the block is filled with DC value */
...
JRESULT jd_mcu_output(JDEC *jd, /* Pointer to the decompressor object */
...
JRESULT
jd_prepare(JDEC *jd, /* Blank decompressor object */
           size_t (*infunc)(JDEC *, uint8_t *,
                            size_t), /* JPEG stream input function */
...
JRESULT jd_decomp(JDEC *jd, /* Initialized decompression object */
  ...
  for (y = 0; y < jd->height; y += my) {  /* Vertical loop of MCUs */
    for (x = 0; x < jd->width; x += mx) { /* Horizontal loop of MCUs */
      ...
      rc = jd_mcu_load(jd); /* Load an MCU (decompress huffman coded stream,
