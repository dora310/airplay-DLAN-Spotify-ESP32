...
static bool web_authorized(httpd_req_t *req) {
  ...
  return httpd_req_get_hdr_value_str(req, "X-API-Key", key, sizeof(key)) ==
             ESP_OK &&
         settings_verify_web_password(key);
}
...
static esp_err_t advanced_page_handler(httpd_req_t *req) {
...
static esp_err_t system_info_handler(httpd_req_t *req) {
  ...
  cJSON_AddStringToObject(info, "device_name", SETTINGS_SYSTEM_DEVICE_NAME);
  cJSON_AddStringToObject(info, "airplay_device_name", airplay_device_name);
...
#endif
  cJSON_AddBoolToObject(info, "software_dsp_supported", true);
...
esp_err_t web_server_start(uint16_t port) {
  ...
  err = web_api_v3_register(s_server);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "Failed to register v3 API: %s", esp_err_to_name(err));
    return err;
  }
