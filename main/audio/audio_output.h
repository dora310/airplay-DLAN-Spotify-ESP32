 ...
 * and the music-reactive RGB LED are applied, but the AirPlay volume is not.
 * The caller is responsible for applying its own source volume.
 */
esp_err_t audio_output_write_external_pcm(int16_t *data, size_t frames,
                                          TickType_t wait);

/** Write AirPlay-path PCM with an optional click-free 10 ms edge. */
